...
var focusManage = {
		...
		data_: {
			id: "focusFun",
			url: "<%=rootPath%>/template/modifyTempLatefocus.do",
			data: {} 
			},
		size: {
			preWidth: 0,
		...
		// 改变轮播图样式
		 ...
		 selectAlbum: function(){
			 ...
			 top.layer.open({
				  ...
				  content: ['<%=rootPath%>/jsp/template/manage/pop/selectAlbumPop.jsp?parentName=focusFun', 'no'],
		...
		getDataList: function(){
			$.ajax({
				type : 'post',
				url : "<%=rootPath%>/template/getTempLateFocus.do",
		...
		getPictureList: function(){
			$.ajax({
				type : 'post',
				url : "<%=rootPath%>/template/getPictures.do",
