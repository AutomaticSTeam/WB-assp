	 ...
	 $('#batchDelBtn').on('click', function(){
		 if(checkTableCkSelected()){
			 layer.confirm('您确定删除当前数据', {icon: 3, title:'提示'}, function(index){
				 var ids = getMediaIds();
				 batchEditMedia({dataStatus : 1 , mediaIdsStr : ids }, '删除');
				 //删除数据
				  layer.close(index);
			});
		 }
...
var getMediaIds = function(){
	var mediaIdStr = "";
	 $("input[name='mediaId']:checked").each(function(){
		 mediaIdStr += $(this).val() + ",";
	 });
...
var batchEditMedia = function(data , tipStr){
	 $.ajax({
			type:'post',
			 url:rootPath+'/contents/media/editMediaProperties.do',
			data:data,
			dataType:'json',
			async : true,
			success: function(data){ 
				if(data.rst == 0){
					mediaDataGrid();
					if(tipStr != null){layer.alert(tipStr + '模版数据成功！', {icon: 1});}
				}else{
					if(tipStr != null){layer.alert(tipStr +  '模版数据失败', {icon: 2});}
				}
			 },
			error:function(data){
				//alert(JSON.stringify(data));
...
var checkTableCkSelected = function(){
	var flag = true;
	 if($("input[name='mediaId']:checked").length <= 0){
...
var mediaDataGrid = function(){
	$.ajax({
		type:'post',
		 url:rootPath+'/contents/media/queryMediasByPage.do',
