$(function(){
	...
	$("#saveBtn").click(function(){
		...
		if(frameName!=""&&frameName!=null){
			$.ajax({
				url:rootPath+"/console/frame/editFrameOne.do",
				type:"post",
				datatype:"json",
				data:$(".moduleTmplForm").serialize(),
				success:function(data){
					if(data.code==1){
						window.location.href=rootPath+"/jsp/frame/managerFrame.jsp"
					}else{
	...
	if(frameId==""&&frameId==null){
		...
		if($("#frameAttachmentImg").val()==""||$("#frameAttachmentImg").val()==null){
			$("#thumbImgId").click(function(){
				uploadImg();
			});
		}
