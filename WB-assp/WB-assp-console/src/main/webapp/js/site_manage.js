...
//				  layer.close(index);
   ...
   focusFun: function(current){
  	...
  	//图片区域的编辑属性
  focusPictureFun : function(current){
	  var base=this;
	  layer.open({
		  title: ['傻逼', false],  
	  });
					...
					$moduleBody.find("div[data-model]").each(function(){
						...
						if($current.find(".extendMouduleBtn").length == 0){
							...
							if(dataType != void 0 && dataType.indexOf('1') != -1){
								...
								$current.find(".editMouduleBtn").after('<a class="btn btn-blue white dib lh1 editBtn extendMouduleBtn" data-type="addPicture">添加图册</a>');
							}
						}
					});
