...
var SiteManager = {
	...
	addMouduleBtn: function(editBtn) {
		...
		var $module = $("[data-model]").has(editBtn);
		var id = $module.attr("data-url");
		...
		layer.open({  
            ...
            maxmin: true,  
            title: ['模块类型', false],  
            content: ROOTPATH + '/jsp/website/manage/pop/addModulePop.jsp',// //,
            scrollbar:false,//是否允许滚动条出现
			  //按钮与回调
			btn: ['确认', '取消'],
			yes: function(index, layero){
				if(SiteManager.handers.addModuleFun){
					 SiteManager.handers.addModuleFun.save(index);
				 }
			},
			cancel: function(index){
				SiteManager.handers.addModuleFun = null;
				return true;
			}
			}); 
	},
	...
	editMouduleBtn: function(editBtn) {
		...
		var styleShow = $module.attr("style-show");
		if(dataShow == void 0){
			return;
		}
		...
		if(dataType != void 0 && dataType != null){
			var dataType_2 = dataType.split("/");
			dataType_1 = dataType_2[0] == "0" ? dataType_2[1] : dataType_2[0];
		}
		switch(dataType_1){
		// logo编辑
		case "10":
		...
		// 文章内容页编辑	
		case "2":
			var artticle_model = $module.find("div[data-type='article']").attr("data-url");
			if(artticle_model != void 0){
				dataUrl = artticle_model;
			}
		...
		// 图片内容页编辑	
		case "7":
			this.contentPicture(dataUrl);
			break;
		default: 
			if(styleShow != void 0 && styleShow.indexOf("01") != -1){//轮播图
	...
	editBtn: function(editBtn) {
		...
		switch (dataType) {
			case "picture" :
				SiteManager.pictureFun(dataId,dataUrl);
				break;
			case "article" :
	...
	delBtn: function(delBtn) {
				//只是针对图片删除不对应文章和视频
				var $module = $("div[data-model]").has(delBtn);
				var dataUrl = $module.attr("data-url");		
				...
				switch (dataType) {
					...
					case "picture" :
						SiteManager.pictureDel(dataId,dataUrl);
						break;
					case "article" :
...
//				  layer.close(index);
   ...
   focusFun: function(current){
	...
	// 页面中图层编辑按钮
	pictureFun: function(current,dataUrl){
		$.ajax({
			...
			success:function(data){
				...
				if(albumId != ""){
					...
					layer.open({  
			            ...
			            maxmin: true,  
			            title: ['编辑图册', false],  
			            content: ROOTPATH + '/jsp/website/manage/pop/addPicturePop.jsp?siteAblumId='+albumId+'&'+'siteModuleId='+moduleId,
			            scrollbar:false,//是否允许滚动条出现
						}); 
				}
			},
	...
	//删除
	pictureDel : function(current,dataUrl){
		layer.confirm('您确定删除当前数据', {icon: 3, title:'提示'}, function(index){
			batchEditPictureAlbum({albumRelTypeStatus : 1 , pictureId : current , moduleId : dataUrl}, '删除');
			 //删除数据
			  layer.close(index);
		});
	},
	...
	pictureTop : function(current){
		layer.confirm('您确定置顶当前数据', {icon: 3, title:'提示'}, function(index){
			batchEditPictureAlbum({isTop : 1 , pictureId : current } , '置顶');
			 //确认置顶
			  layer.close(index);
		});
	},
			...
			if($moduleBody.find(".operation").length == 0) {
				if($moduleBody.find('[data-model="logo"]').length > 0){
					...
					$moduleBody.find("div[data-model]").each(function(){
						if($(this).attr("data-model") == "addModule"){
							return;
						}
						...
						if($current.find(".extendMouduleBtn").length == 0){
							...
							var styleShow = $current.attr("style-show");
							//给文章模块儿添加添加文章按钮
							...
							}else if(dataType != void 0 && dataType.indexOf('/8/') != -1){
								$current.find(".editMouduleBtn").after('<a class="btn btn-blue white dib lh1 editBtn extendMouduleBtn" data-type="addMedia">添加视频</a>');
							}else if(dataType != void 0 && (dataType.indexOf('/5/') != -1) && (styleShow == void 0 || styleShow.indexOf("01") == -1) ){
	...
	contentPicture:function(id){
		layer.open({  
            ...
            maxmin: true,  
            title: ['编辑图片', false],  
            content: ROOTPATH + '/site/toPictureEditOrView.do?pictureId='+id,
            scrollbar:false,//是否允许滚动条出现
			}); 
	}
...
var batchEditPictureAlbum = function(data , tipStr){
	 $.ajax({
			type:'post',
			 url:rootPath+'/site/editPictureAlbumProperties.do',
			data:data,
			dataType:'json',
			async : true,
			success: function(data){ 
