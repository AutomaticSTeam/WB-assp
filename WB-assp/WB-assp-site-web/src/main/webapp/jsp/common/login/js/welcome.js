					...
					success:function(data){
						if(data.length<1){
							...
							for (var i = 0; i < data.length; i++) {
								...
								"	<a class='admin-btn' data-url="+data[i].siteId+" id="+data[i].templateId+" onclick='manageSite(this.id)'>管理</a>\n" +
								"</li>"
...
function manageSite(templateId){
	var siteId=$("#"+templateId).attr("data-url");
	if(templateId!=null&&templateId!=""){
		//查询当前站点的模板与首页id
		$.ajax({
			url:rootPath+"/site/getWebsiteInfoByTemplate.do",
			type:"post",
			datatype:"json",
			data:{
			...
			success:function(data){
				...
				if(columnsId!=null){
					location.href = rootPath + "/site/loadWebsitePage.do?siteId="+siteId+"&templateId="+templateId+"&columnsId="+columnsId+"";
				}
			},
			error:function(){
