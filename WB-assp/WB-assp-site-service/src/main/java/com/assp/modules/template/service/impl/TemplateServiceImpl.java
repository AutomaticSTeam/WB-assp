...
public class TemplateServiceImpl implements ITemplateService {
	...
	public List<TemplateRelFrame> queryPageFrameInfo(Template template,Integer columnsId) {
		...
		List<TemplateRelFrame> templateRelFrames = null;
		
		//获取公共加载模块儿
		if(template != null && template.getTemplateId() != null){
			templateRelFrames = new ArrayList<TemplateRelFrame>();
			TemplateCommonRelFrame tcr = new TemplateCommonRelFrame();
			tcr.setTemplateId(template.getTemplateId());
			tcr.setSiteId(template.getSiteId());
		...
		if(columnsId != null ){
			if(templateRelFrames == null) {templateRelFrames = new ArrayList<TemplateRelFrame>();}
