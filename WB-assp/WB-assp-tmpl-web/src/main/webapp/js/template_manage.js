...
var Web = {
	...
	other: function() {
		...
		base.listOperationBtn2New('[data-model="product"] li');
		//网站底部
...
/*		var base = this,
			$this = $(current),
			urlValue = base.rootPath + '/jsp/template/manage/logoEdit.jsp', // 设置ifarme地址
			popIfarmeWidth = '474', // 设置ifarme宽度
			popIfarmeHeight = '260', // 设置ifarme高度
			id = 'formModel-1';

		base.iframePop(base.$popTitle, urlValue, popIfarmeWidth, popIfarmeHeight, id, 'logoPop'); // 添加logo网站标志操作弹出框
		
		// 鼠标点击确定按钮
		$('#' + id).find('.confirm-btn').on('click', function() {
			var $obtainContent = $('#' + id).find('.pop-iframe').find('.obtainContent'); // 获取iframe内的class="obtainContent";
			
			$this.parent().parent().parent().find('img').attr({
				'src': $obtainContent.attr('src'), // 获取图片地址
				'alt': $obtainContent.attr('alt'), // 获取图片名字
				'title': $obtainContent.attr('alt'), // 获取图片标题
				'width': $obtainContent.width(), // 获取图片宽带
				'height': $obtainContent.height() // 获取图片高度
			});
		});
...
var websiteManage = {
		...
		Class : function() {
		    ...
		    if (len > 1) {
		        ...
		        websiteManage.inherit.apply(null, newArgs);
		    } else {
		        C.prototype = F;
		    }
		    return C;
